...
10. Modal behavior verification (CRITICAL - must verify after implementation):
    - Focus trap: When dialog opens, focus must move to close button. Tab key must cycle within dialog only (not escape to page behind).
    - Body scroll lock: Set document.body.style.overflow = 'hidden' when visible=true, reset to '' when visible=false.
    - Backdrop click: Clicking outside image (on dialog backdrop) must close the modal.
    - ESC key: Pressing Escape must close the modal (native <dialog> behavior, but verify).
    - Return focus: After closing, focus should return to the thumbnail that opened the lightbox.
  </action>
  <verify>
    Component exists at components/GalleryLightbox.client.vue with Teleport, dialog element, close handler, proper props interface, and modal behavior code (focus trap, body scroll lock, backdrop click, ESC key handling).
  </verify>
  <done>
    GalleryLightbox component renders modal with image, close button, and backdrop. Implements focus trap, body scroll lock, backdrop-click-close, ESC-close, and return-focus behaviors.
  </done>
</task>
